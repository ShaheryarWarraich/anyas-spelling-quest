...
import SkipRuleScreen from './ui/screens/SkipRuleScreen.jsx';
...
export default function App() {
  ...
  if (view.name === 'skiprule') return <SkipRuleScreen app={app} home={home} onHome={goHome} />;
  ...
  return <HomeScreen app={app} home={home} onPlay={() => play()} onNext={() => play({ next: true })} onRedo={() => setView({ name: 'redo' })} onSkipRule={() => setView({ name: 'skiprule' })} onRemember={() => setView({ name: 'revision' })} onParent={() => setView({ name: 'parent' })} onBadge={week => setView({ name: 'badge', week })} />;
}
