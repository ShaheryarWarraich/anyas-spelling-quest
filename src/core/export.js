...
export function toCSVExport(dump) {
  const sessions = csv(['date', 'weekId', 'dayType', 'status', 'minutes', 'countsForStreak', 'waysDone', 'switches', 'wordsCorrect', 'wordsTotal', 'transferCorrect', 'transferTotal', 'probeCorrect', 'probeTotal', 'selfCaught', 'exceptionCorrect', 'exceptionTotal', 'earCorrect', 'earTotal', 'pickCorrect', 'pickTotal', 'sentencesCorrect', 'sentencesTotal', 'bonus'],
    dump.days.map(d => {
  ...
  const words = csv(['date', 'weekId', 'word', 'source', 'ruleId', 'way', 'mark', 'ruleTap', 'sentence'],
    dump.days.flatMap(d => [...(d.wayWords || []), ...d.words, ...(d.sentences || [])].map(w => ({ date: d.date, weekId: d.weekId, word: w.word, source: w.source, ruleId: w.ruleId, way: w.way ?? '', mark: w.mark === null || w.mark === undefined ? '' : w.mark, ruleTap: w.ruleTap || '', sentence: w.sentence || '' }))));
  ...
  const ear = csv(['date', 'weekId', 'kind', 'heard', 'options', 'chosen', 'correct'],
    dump.days.flatMap(d => [...(d.ear || []).map(e => ({ date: d.date, weekId: d.weekId, kind: 'word', heard: e.word, options: e.options.join(' | '), chosen: e.chosen ?? '', correct: e.chosen == null ? '' : e.correct })),
