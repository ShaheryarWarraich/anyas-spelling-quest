...
export default function HomeScreen({ app, home, onPlay, onRemember, onParent, onBadge }) {
  const { info, streak, todayDay } = home;
  const stickers = home.stickers.map(s => s.id);
  const week = info.weekEntry?.week;
  let main;
  if (info.dayType === 'rest') main = <><div className="rulecard"><div className="name">🌿 Rest day</div><div className="text">No spelling today. See you tomorrow!</div></div></>;
