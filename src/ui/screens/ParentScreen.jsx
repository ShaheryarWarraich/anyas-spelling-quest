    ...
    <div className="screen parent">
      ...
      {tab === 'ear' && <>
        ...
        <table><thead><tr><th>Date</th><th>Week</th><th>Heard</th><th>Type</th><th>Tapped</th><th>Right?</th></tr></thead><tbody>
          {d.weeks.flatMap(w => w.sessions.flatMap(s => [...s.earItems.map((e, i) => <tr key={s.date + 'e' + i}><td>{s.date}</td><td>{w.weekId}</td><td>{e.word}</td><td>{e.source === 'exception' ? 'rule breaker' : 'word'}</td><td>{e.chosen ?? '–'}</td><td>{e.chosen == null ? '–' : e.correct ? '✓' : <span className="tag no">✗</span>}</td></tr>),
            ...s.pickItems.map((e, i) => <tr key={s.date + 'p' + i}><td>{s.date}</td><td>{w.weekId}</td><td>{e.text}</td><td>sentence ({e.target})</td><td>{e.chosen ?? '–'}</td><td>{e.chosen == null ? '–' : e.correct ? '✓' : <span className="tag no">✗</span>}</td></tr>)]))}
        </tbody></table></>}
